...
create table if not exists public.field_change_events (
  id bigserial primary key,
...
alter table public.field_change_events
...
update public.field_change_events
...
create table if not exists public.notification_reads (
  ...
  change_event_id bigint not null references public.field_change_events(id) on delete cascade,
  read_at timestamptz not null default now(),
  unique (user_id, change_event_id)
);

create index if not exists field_change_events_changed_at_idx
  on public.field_change_events (changed_at desc);

create index if not exists field_change_events_identity_field_idx
  on public.field_change_events (table_name, record_identity, field_name, changed_at desc);

create index if not exists field_change_events_table_changed_idx
  on public.field_change_events (table_name, changed_at desc);

create index if not exists field_change_events_record_uuid_idx
  on public.field_change_events (record_uuid, changed_at desc);
